	...
	dajax: function (method, url, data, func, errorfunc, contentType) {
		jQuery.ajax({
			...
			success: function (data) {
				...
				if (sys_error !== '') {
					if (typeof errorfunc === 'function') {
						var siteName = typeof SITE_INFO.siteName !== 'undefined' ? SITE_INFO.siteName : '微社區';
						if (sys_error === 'mobile_is_closed') {
							TOOLS.showError('.warp', '您請求的' + siteName + '無法訪問<br /><br />該社區未啟用手機版');
						} else {
							TOOLS.showError('.warp', '您請求的' + siteName + '無法訪問<br /><br />接口錯誤(ERR01)');
						}
					} else {
						return;
					}
			...
			error: function (data) {
				...
				if (typeof errorfunc === 'function') {
					var siteName = typeof SITE_INFO.siteName !== 'undefined' ? SITE_INFO.siteName : '微社區';
					TOOLS.showError('.warp', '您請求的' + siteName + '無法訪問<br /><br />接口錯誤(ERR02)');
					TOOLS.hideLoading();
				}
			}
		});
			...
			TOOLS.dajax('GET', API_URL + 'module=check&version=4&_t=' + t, null, function (re) {
				if (typeof (re) == "string") {
					try {
						...
						var siteName = typeof SITE_INFO.siteName !== 'undefined' ? SITE_INFO.siteName : '微社區';
						TOOLS.showError('.warp', '您請求的' + siteName + '無法訪問<br /><br />接口錯誤(ERR03)');
						TOOLS.hideLoading();
						return;
					}
				}
					...
					TOOLS.dajax('GET', API_URL + 'module=wsqindex&version=4&_t=' + t, null, function (re) {
						if (typeof (re) === "string") {
							try {
								...
								var siteName = typeof SITE_INFO.siteName !== 'undefined' ? SITE_INFO.siteName : '微社區';
								TOOLS.showError('.warp', '您請求的' + siteName + '無法訪問<br /><br />接口錯誤(ERR04)');
								TOOLS.hideLoading();
								return;
							}
						}
				...
				jQuery.ajax({type: 'GET', url: API_URL + 'module=checkcookie&version=4&_t=' + t, contentType: 'application/x-www-form-urlencoded', xhrFields: {withCredentials: true},
					...
					error: function (data) {
						...
						if (matches != null) {
							...
							var siteName = typeof SITE_INFO.siteName !== 'undefined' ? SITE_INFO.siteName : '微社區';
							TOOLS.showError('.warp', '您請求的' + siteName + '無法訪問<br /><br />接口錯誤(ERR05)');
							TOOLS.hideLoading();
						}
					}
				});
