...
$return .= <<<EOF
, the hidden content of this post requires credits higher than {$creditsrequire} to browse, your current credits are {$_G['member']['credits']}</div>
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
<div class="locked">The following content requires credits higher than {$creditsrequire} to browse</div>
{$message}<br /><br />
...
?><?php
$return = <<<EOF
<div class="blockcode"><div id="{$randomid}"><ol><li>{$code}</ol></div><em onclick="copycode($('{$randomid}'));">Copy code</em></div>
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
<div class="showhide"><h4>Hide the contents of this post</h4>\\1</div>
...
$return .= <<<EOF
, if you want to view the hidden content of this post, please <a href="forum.php?mod=post&amp;action=reply&amp;fid={$_G['fid']}&amp;tid={$_G['tid']}" onclick="showWindow('reply', this.href )">Reply</a></div>
EOF;
?><?php 
return $return;
}
...
$return .= <<<EOF
, Download times: {$attach['downloads']}
EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
, Download credits: {$_G['getattachcredits']}
EOF;
 } 
$return .= <<<EOF
</p>
...
Selling price: <strong>{$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}</strong> &nbsp;[<a href="forum.php?mod=misc&amp;action=viewattachpayments&amp;aid={$attach['aid']}" onclick="showWindow('attachpay', this.href)" target="_blank">Record</a>]

EOF;
 if(!$attach['payed']) { 
$return .= <<<EOF

&nbsp;[<a href="forum.php?mod=misc&amp;action=attachpay&amp;aid={$attach['aid']}&amp;tid={$attach['tid']}" onclick="showWindow('attachpay', this.href)">Buy</a>]
...
<em class="xg1">({$attach['attachsize']}, Download times: {$attach['downloads']})</em>
</p>
<div class="tip tip_4" id="aid{$attach['aid']}_menu" style="display: none" disautofocus="true">
<div>
<p>
<a href="forum.php?mod=attachment{$is_archive}&amp;aid={$aidencode}&amp;nothumb=yes" target="_blank">Download attachments</a>

EOF;
 if(helper_access::check_module('album')) { 
$return .= <<<EOF

&nbsp;<a href="javascript:;" onclick="showWindow(this.id, this.getAttribute('url'), 'get', 0);" id="savephoto_{$attach['aid']}" url="home.php?mod=spacecp&amp;ac=album&amp;op=saveforumphoto&amp;aid={$attach['aid']}&amp;handlekey=savephoto_{$attach['aid']}">Save into the album</a>

EOF;
 } if($firstpost && $_G['fid'] && $_G['forum']['picstyle'] && ($_G['forum']['ismoderator'] || $_G['uid'] == $attach['uid'])) { 
$return .= <<<EOF

&nbsp;<a href="forum.php?mod=ajax&amp;action=setthreadcover&amp;aid={$attach['aid']}&amp;fid={$_G['fid']}" onclick="showWindow('setcover{$attach['aid']}', this.href)">Set as cover</a>
...
<p>
<span class="y">{$attach['dateline']} Upload</span>
...
&nbsp;[<a href="forum.php?mod=misc&amp;action=attachpay&amp;aid={$attach['aid']}&amp;tid={$attach['tid']}" onclick="showWindow('attachpay', this.href)" target="_blank">Buy</a>]
...
<br>
<a href="member.php?mod=logging&amp;action=login" onclick="showWindow('login', this.href+'&referer='+encodeURIComponent(location));">Sing in / Register to see the picture</a>
</div>
...
$return .= <<<EOF
 aid="{$attach['aid']}" src="{$__STATICURL}image/common/none.gif" zoomfile="
EOF;
 if($attach['refcheck']) { 
$return .= <<<EOF
forum.php?mod=attachment{$is_archive}&aid={$aidencode}&noupdate=yes&nothumb=yes
...
<div class="xs0">
<p><strong>{$attach['filename']}</strong> <em class="xg1">({$attach['attachsize']}, Download times: {$attach['downloads']})</em></p>
<p>
<a href="forum.php?mod=attachment{$is_archive}&amp;aid={$aidencode}&amp;nothumb=yes" target="_blank">Download attachments</a>
...
<p class="xg1 y">{$attach['dateline']} Upload</p>
...
<em class="xg1">({$attach['attachsize']}, Download times: {$attach['downloads']}
EOF;
 if($attach['price']) { 
$return .= <<<EOF
, Selling price: {$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}
EOF;
 } 
$return .= <<<EOF
)</em>
...
<div class="tip_c xs0">
<div class="y">{$attach['dateline']} Upload</div>
...
$return .= <<<EOF
<br />Read permission: {$attach['readperm']}
EOF;
 } if(IS_ROBOT) { 
$return .= <<<EOF
...
<br />Selling price: {$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}	&nbsp;<a href="forum.php?mod=misc&amp;action=viewattachpayments&amp;aid={$attach['aid']}" onclick="showWindow('attachpay', this.href)" target="_blank">[Record]</a>
...
$return .= <<<EOF
<br />Download credits: {$_G['getattachcredits']}
EOF;
 } 
$return .= <<<EOF
